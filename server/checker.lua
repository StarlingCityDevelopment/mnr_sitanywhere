...
assert(
	GetResourceState("ox_lib") == "started",
	"ox_lib not found or not started before this script, install or start before ox_lib"
)
...
AddEventHandler("onResourceStart", function(resourceName)
	...
	assert(
		GetCurrentResourceName() == expectedName,
		("The resource name is incorrect. Please set it to %s.^0"):format(expectedName)
	)
end)
